...
void stb_vorbis_channels_short_interleaved(int buf_c, short *buffer, int data_c, float **data, int d_offset, int len)
{
   int i;
   check_endianness();
   if (buf_c != data_c && buf_c <= 2 && data_c <= 6) {
...
int stb_vorbis_get_frame_short_interleaved(stb_vorbis *f, int num_c, short *buffer, int num_shorts)
{
   ...
   if (len) {
      ...
      stb_vorbis_channels_short_interleaved(num_c, buffer, f->channels, output, 0, len);
   }
   return len;
}
...
int stb_vorbis_get_samples_short_interleaved(stb_vorbis *f, int channels, short *buffer, int num_shorts)
{
   ...
   while (n < len) {
      ...
      if (n+k >= len) k = len - n;
      if (k)
         stb_vorbis_channels_short_interleaved(channels, buffer, f->channels, f->channel_buffers, f->channel_buffer_start, k);
      buffer += k*channels;
      n += k;
