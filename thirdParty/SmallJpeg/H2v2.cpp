...
void jpeg_decoder::H2V2Convert(void)
{
  ...
  static __m64 zero = _m_from_int(0);
#endif
  ...
  if (use_mmx)
  {
    ...
    for (int i = max_mcus_per_row; i > 0; i--)
    {
      for (int l = 0; l < 2; l++)
      {
        ...
        cb = _m_psllw(_m_punpcklbw(cb, zero), _m_from_int(2));

        cr = m3;
        cr = _m_punpcklbw(cr, cr);
        cr = _m_psllw(_m_punpcklbw(cr, zero), _m_from_int(2));
        ...
        y1 = _m_punpcklbw(_m_psrlq(yy, _m_from_int(32)), zero);
